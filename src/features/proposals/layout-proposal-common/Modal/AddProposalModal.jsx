...
import { faPlus, faTrash, faSearch, faFilePdf } from "@fortawesome/free-solid-svg-icons";
...
export default function AddProposalModal() {
  ...
  const submit = async (e) => {
    ...
    const fallbackTitle =
      (teamContext?.team?.teamCode && `Proposal ${teamContext.team.teamCode}`) ||
    ...
    <div className={styles.overlay} onClick={closeModal}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h3>Tạo đồ án mới</h3>
          <p className={styles.subtitle}>Nhập Team ID để tự động lấy thông tin nhóm</p>
        </div>
        ...
        <form className={styles.body} onSubmit={submit}>
            ...
            <div className={styles.cardHeader}>
              <div className={styles.teamTitle}>
                <span className={styles.teamCode}>
                  {teamContext?.team?.teamCode || teamContext?.team?.teamId || "—"}
                </span>
                <span
                  className={`${styles.badge} ${
                    teamContext?.existingProposal?.title ? styles.badgeSuccess : styles.badgeWarn
                  }`}
                >
          ...
          <div className={styles.group}>
            <div className={styles.groupHeader}>
              <h4>Mục tiêu</h4>
              <button type="button" className={styles.ghostBtn} onClick={() => addRow(setGoals, "")}>
          ...
          <div className={styles.group}>
            <div className={styles.groupHeader}>
              <h4>Công nghệ</h4>
              <button type="button" className={styles.ghostBtn} onClick={() => addRow(setTechnologies, "")}>
            ...
            {technologies.map((t, i) => (
              <div className={styles.lineRow} key={i}>
                <input
                  ...
                  value={t}
                  onChange={(e) => handleArrChange(setTechnologies, i, e.target.value)}
                />
                <button
                  type="button"
                  className={styles.iconBtn}
