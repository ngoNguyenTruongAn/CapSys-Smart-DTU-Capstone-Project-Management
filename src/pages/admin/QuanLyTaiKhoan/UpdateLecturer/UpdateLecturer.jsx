...
const UpdateLecturer = ({ show, setShow, lecturerId }) => {
  ...
  const validateForm = () => {
    ...
    if (/\s/.test(lecturerCode)) {
      pushError("Mã giảng viên không được chứa khoảng trắng!");
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToastSuccess("Cập nhật giảng viên thành công");
      setShow(false);
    } catch (err) {
      pushError("Cập nhật thất bại: " + (err?.message || err));
    }
  };
