...
const SwapStudentModal = ({
  ...
  const pushError = (msg) => setToastErrors((prev) => [...prev, msg].slice(-3)); // giữ tối đa 3 lỗi

  const handleSwap = async () => {
    if (!selectedStudentId1 || !selectedStudentId2) {
      pushError("Vui lòng chọn 2 sinh viên để hoán đổi!");
      return;
    }
    ...
    try {
      ...
      if (swapStudents.fulfilled.match(result)) {
        setToastSuccess("Hoán đổi sinh viên thành công!");
        ...
        pushError("Lỗi khi hoán đổi sinh viên!");
      }
    } catch (error) {
      console.error("Swap error:", error);
      pushError("Đã xảy ra lỗi khi gọi API hoán đổi!");
    }
  };
